...
exam_grades = pd.read_csv('hw_exam_grades.csv', converters={"SID": str.lower},
...
roster = pd.read_csv('roster.csv', converters={"StudentID": str.lower, "Email Address": str.lower},
                     usecols=["Group", "Email Address", "StudentID"], )

# DATA CLEANING
final_data = pd.merge(
    roster, exam_grades, left_index=True, right_index=True,
)
final_data = pd.merge(
...
final_data = final_data.drop(columns=['StudentID_y'])
...
final_data = final_data.sort_values(by='Ceiling Score', ascending=False)
...
for section, table in final_data.groupby("Group"):
    section_file = HERE / f"Section {section} Grades.csv"
